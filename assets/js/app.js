...
function closeModal(){if(!$('modal').classList.contains('open'))return;$('modal').classList.remove('open');document.body.style.overflow='';history.replaceState(null,'','#'+(document.querySelector('.view.active')?.id||'registry'));returnFocus?.focus();window.SiteAnalytics?.view('/section/'+(document.querySelector('.view.active')?.id||'registry'),'Registry');}
...
function tab(id){if(!document.querySelector(`.tab[data-view="${id}"]`))return;window.SiteAnalytics?.view('/section/'+id,id);document.querySelectorAll('.tab').forEach(t=>{const on=t.dataset.view===id;t.classList.toggle('active',on);t.setAttribute('aria-pressed',String(on));});document.querySelectorAll('.view').forEach(v=>v.classList.toggle('active',v.id===id));}
