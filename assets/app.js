...
const display=v=>absent(v)?'未公开':String(v);
...
function countryLabel(code){if(!code)return'未标注';try{return new Intl.DisplayNames(['zh-CN'],{type:'region'}).of(code)+' · '+code;}catch{return code;}}
...
function market(){const items=records.filter(r=>r.tier==='chassis'||r.id==='SP3-DK-2400'||r.id==='SP3-CARRIO-BIANCO-ITALIA'||r.id==='SP3-BP-GREEN');$('marketRows').innerHTML=items.map(r=>`<tr><td><button data-id="${esc(r.id)}">${esc(r.title)}</button><br><small>${esc(r.id)}</small></td><td>${esc(display(r.status))}</td><td>${esc(display(r.sale))}</td><td><a href="${esc(safeURL(r.sources[0][1]))}" target="_blank" rel="noopener noreferrer">原始来源 ↗</a><br><small>${esc(r.review_status)}</small></td></tr>`).join('');bindCards($('marketRows'));}
...
function countryStats(){bars('countryBars',records.filter(r=>r.record_kind!=='prototype'&&!absent(r.country)).map(r=>r.country));bars('indexCountryBars',sourceIndex.map(r=>countryLabel(r.country_code)));}
...
function gallery(){
 const r=current,photos=r.photos||[];
 if(!photos.length){$('gallery').innerHTML=art(r);return;}
 const source=safeURL(r.photo_sources?.[gidx]||r.sources[0]?.[1]);
 $('gallery').innerHTML=`<div class="gallery-main"><img class="gallery-image" src="${esc(safeURL(photos[gidx],true))}" alt="${esc(r.title+' · '+(r.photo_captions?.[gidx]||('照片 '+(gidx+1))))}"><div class="gnav"><button id="prev" aria-label="上一张">‹</button><button id="next" aria-label="下一张">›</button></div></div><div class="gallery-caption"><span>${gidx+1} / ${photos.length} · ${esc(r.photo_captions?.[gidx]||'原始图库')}</span><span>${esc(r.credits[gidx]||'版权归原摄影者')} · <a href="${esc(source)}" target="_blank" rel="noopener noreferrer">本图来源 ↗</a> · <a href="${esc(safeURL(photos[gidx],true))}" target="_blank" rel="noopener noreferrer">打开原图 ↗</a></span></div><div class="filmstrip" aria-label="选择照片">${photos.map((p,i)=>`<button class="film ${i===gidx?'selected':''}" data-image="${i}" aria-label="第 ${i+1} 张照片" aria-pressed="${i===gidx}"><img loading="lazy" src="${esc(safeURL(p,true))}" alt="${i+1}"><span>${i+1}</span></button>`).join('')}</div>`;
...
function detail(){const r=current,fields=[['年份',r.year],['VIN',r.vin],['底盘',r.chassis],['生产归类',r.edition],['公开国家',r.country],['公开地点',r.city],['市场规格',r.market],['公开车主／收藏',r.owner],['外观',r.exterior],['内饰',r.interior],['轮毂',r.wheels],['卡钳',r.calipers],['里程（来源时点）',r.mileage],['状态（来源时点）',r.status],['价格／结果',r.sale],['定制项目',r.program]];$('detail').innerHTML=`<div class="dhead"><div><h2>${esc(r.title)}</h2><div class="sub">${esc(tierNames[r.tier])} · ${kindNames[r.record_kind]}</div></div></div><div class="notice">${esc(r.review_status||'导入资料；未独立核验')}<br>无 VIN 的档案可能与其他来源重复。公开地点不代表当前车主居住地。</div><div class="dgrid">${fields.map(([k,v])=>`<div><label>${k}</label><span>${esc(display(v))}</span></div>`).join('')}</div><div class="dcols"><div><h3>配置与选装</h3><ul class="clean">${r.options.length?r.options.map(x=>`<li>${esc(x)}</li>`).join(''):'<li>尚无公开配置明细。</li>'}</ul></div><div><h3>公开时间线</h3><div class="timeline">${r.timeline.length?r.timeline.map(x=>`<p>${esc(x)}</p>`).join(''):'<p>尚无可核验时间线。</p>'}</div></div></div><h3>研究备注</h3><p class="clean">${esc(r.notes||'暂无补充备注。')}</p><h3>逐车来源</h3><div class="links">${r.sources.map(([label,url])=>`<a href="${esc(safeURL(url))}" target="_blank" rel="noopener noreferrer">${esc(label)} ↗</a>`).join('')}</div><p><button class="btn" id="copyLink">复制此车链接</button></p>`;$('copyLink').onclick=async()=>{try{await navigator.clipboard.writeText(location.href);toast('链接已复制');}catch{toast('请复制浏览器地址栏中的链接');}};}
