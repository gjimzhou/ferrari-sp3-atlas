...
- **51 条有来源的研究记录**：恢复历史 v3 的 48 条记录，并补入 3 条经销商车档。详细档案、待核对线索、预生产车分开显示。
- **503 个 ECR 公开档案索引**：由公开车型搜索 21 页结果去重所得，保存链接、缩略图引用和国家旗标。没有复制登录后字段。
- **132 张图库照片引用**：Leclerc／Sainz 各 5 张，三辆 RM 拍卖车分别 21／32／47 张；可用缩略图、方向键或触摸滑动切换，逐张查看来源。
- **4 个完整公开 VIN**：构建时验证格式及唯一性。
- 已复核 RM Sotheby’s 底盘 **297567：USD 17,825,000**、**299827：CHF 5,348,750** 的公布成交结果。
- Leclerc、Sainz、Ramsay、Ronaldo、Ibrahimović、Piero Ferrari、David Lee 等已有档案可搜索。历史收藏关系不代表当前持有人。

**51 与 503 不能相加为实车总数。** 不同来源可能指向同一辆车；未匹配 VIN 的网页编号只代表研究记录。历史条目尚未全量重新核验，在详情页保留核对状态。旧版数百条空白总体占位不再出现在新版数据中。
